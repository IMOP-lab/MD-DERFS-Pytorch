...
class DconnNet(nn.Module):
    ...
    def forward(self, x):
        ...
        c1 = self.backbone.relu(x)#1/2  64           c1=   x->conv1->bn->relu
        
        x = self.backbone.maxpool(c1)                
        c2 = self.backbone.layer1(x)#1/4   64           #c2 = x->conv1->bn->relu->maxpool->layer1
        c3 = self.backbone.layer2(c2)#1/8   128         #c3 = x->conv1->bn->relu->maxpool->layer1->layer2
        c4 = self.backbone.layer3(c3)#1/16   256        #c4 = x->conv1->bn->relu->maxpool->layer1->layer2->layer3
        c5 = self.backbone.layer4(c4)#1/32   512        #c5 = x->conv1->bn->relu->maxpool->layer1->layer2->layer3->layer4

        #### directional Prior ####
        directional_c5 = self.channel_mapping(c5)       #channel_mapping = conv + batchnorm + relu
